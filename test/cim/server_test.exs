defmodule Cim.ServerTest do
  ...
  describe "get_data/2" do
    test "returns the correct value for the database and key", %{pid: pid} do
      ...
      assert {:ok, "test"} = Server.get_data(pid, %{database_name: "test_database", key: "test_key"})
    end

    test "returns thand error if the database/key is not present", %{pid: pid} do
      assert {:data_not_found, "The database or key do not exist"} =
               Server.get_data(pid, %{database_name: "test_database", key: "test_key"})
    end
  end
  ...
  describe "delete_key/2" do
    test "deletes the correct key", %{pid: pid} do
      ...
      assert {:ok, :key_deleted} =
               Server.delete_key(pid, %{database_name: "test_database", key: "test_key"})

      assert {:ok, "test"} =
               Server.get_data(pid, %{database_name: "test_database", key: "test_key_two"})

      assert {:ok, "test"} =
               Server.get_data(pid, %{database_name: "test_database_two", key: "test_key"})
  ...
  describe "delete_database/2" do
    test "deletes the correct database", %{pid: pid} do
      ...
      assert {:ok, :database_deleted} =
               Server.delete_database(pid, %{database_name: "test_database"})
